...
export async function updateCommunityPageButtonsUI(communityIdParam: string) {
    ...
    if (token) {
        ...
        communityContainers.forEach((container, index) => {
            ...
            const isAdmin = userCommunityData.some(item => item.communityId === communityId && item.role === "Administrator");

            if (communityId === communityIdParam) {
